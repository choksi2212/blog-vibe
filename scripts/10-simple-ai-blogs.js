...
/**
 * Simple AI Blog Generator - OpenRouter API
 * Uses OpenRouter API with DeepSeek model for reliable blog generation
 */
...
if (!OPENROUTER_API_KEY) {
  ...
  console.log('Please add OPENROUTER_API_KEY=your_api_key to your .env.local file');
  process.exit(1);
}
...
const openai = new OpenAI({
  ...
  defaultHeaders: {
    "HTTP-Referer": "https://devnovate-blog.vercel.app", // Your site URL
    "X-Title": "Devnovate Blog Platform", // Your site name
  },
});

// Simple blog topics
const topics = [
    "UPI 3.0: How India is Redefining Global Digital Payments",
    "ONDC Explained: The Future of E-Commerce in India",
    "Building Apps for Bharat: Offline-First and Low-Bandwidth Design", 
...
async function generateMultipleBlogs(count = 5) {
  console.log(`🚀 Starting Simple AI Blog Generation...`);
  console.log(`📊 Generating ${count} blogs with reliable parsing`);
  console.log('════════════════════════════════════════════════════════════');
  ...
  console.log('\n🎉 Simple blog generation completed!');
